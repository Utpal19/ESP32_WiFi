...
// Queue handle used to manuplate the main queue of events
 ...
 * @param pvParameters parameter which cn be passed to the task
*/
        ...
        if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
        {
            switch (msg.msgID)
            {
                case WIFI_APP_MSG_START_HTTP_SERVER:
                    ...
                    // http_server_start();
                    ssd1306_display_text(&dev, 3, "WiFi Server Started", 20, true);
                    break;
                ...
                case WIFI_APP_MSG_STA_CONNECTED_GOT_IP:
                    ...
                    ssd1306_display_text(&dev, 4, "WiFi STA IP", 12, true);
...
{
    const char* wifi_start_msg = "Booting WiFi";
    ESP_LOGI(TAG, "STARTING WIFI APPLICATION");
    display_wifi_app_msg(&dev, wifi_start_msg, strlen(wifi_start_msg));

    // Disable the default WiFi logging messages
